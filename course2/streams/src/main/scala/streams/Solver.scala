...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = for {
    bn <- b.legalNeighbors.toStream
  } yield (bn._1, bn._2 :: history) 
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      ...
      case x#::xs => {
        val nbs = neighborsWithHistory(x._1, x._2)
        val nbsnew = newNeighborsOnly(nbs, explored)
        val expl = explored ++ nbsnew.map(n=>n._1)
        x #:: from(initial.tail ++ nbsnew, expl)
      }
    }
  }
