...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val massX: Float = bodies.map(x => (x.mass * x.x)).sum / mass
    val massY: Float = bodies.map(y => (y.mass * y.y)).sum / mass
    val total: Int = bodies.length
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val hsize = size / 2
        val qsize = size / 4
        var f = Fork(Empty(centerX - qsize, centerY - qsize, hsize), Empty(centerX + qsize, centerY - qsize, hsize), Empty(centerX - qsize, centerY + qsize, hsize), Empty(centerX + qsize, centerY + qsize, hsize))
        for (x <- b +: bodies) f = f.insert(x)
        f
      } else {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          val d = distance(x, y, quad.massX, quad.massY)
          if (quad.size / d < theta) addForce(quad.mass, quad.massX, quad.massY)
          else {
            traverse(nw)
            traverse(ne)
            traverse(sw)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = (Math.min(Math.max(b.x - boundaries.minX, 0) / sectorSize, sectorPrecision - 1)).toInt
      val y = (Math.min(Math.max(b.y - boundaries.minY, 0) / sectorSize, sectorPrecision - 1)).toInt
      this(x, y) += b
      this
    }
