...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    output(0) = maxAngle
    for(x <- 1 until input.length) {
      maxAngle = max(input(x)/x, maxAngle)
      output(x) = maxAngle
    } 
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val mid = (end + from) / 2
      //println("upsweep called for mid: " + mid)
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = {
    tree match {
      case Leaf(f, e, _) => downsweepSequential(input, output, startingAngle, f, e)
      case Node(l, r) => 
        //println("downsweep called for starting angle: " + startingAngle)
