...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should work for multi-coins") {
   ...
   test("countChange should return 1 for money = 16 and coins = List(1)") {
    def check(money: Int) = 
      assert(parCountChange(money, List(1), (x, y) => x <= 10.6) == 1,
        s"countChang($money, List()) should be 0")

    check(16)    
  }
   
    test("combinedThreshold should return true for money = 1 and coins = List(5)") {
    ...
    def check(startingMoney: Int,  money: Int) = 
      assert(combinedThreshold(startingMoney, List(5, 6, 7, 8))(money, List(5)) == (money * 1 <= startingMoney * 4 / 2),
        s"combinedThreshold($startingMoney, List(5, 6, 7, 8))($money, List(5)) should be true")
