...
object FunSets {
  ...
  def map(s: Set, f: Int => Int): Set = {
    def mapIter(a: Int, finalSet: Set): Set = {
      if (a < -bound || a > bound) finalSet
      else if (s(a)) {
        if (finalSet == Nil)
          mapIter(a + 1, singletonSet(f(a)))
        else
          mapIter(a + 1, union(finalSet, singletonSet(f(a))))
      } else
        mapIter(a + 1, finalSet)
    }
    mapIter(-bound, Nil)
  }
